...
## M19 — 风格卡与 RAG 风格参考分阶段执行

状态：已完成并通过运行态复验（2026-08-20）

目标：把模板级 Style Card 与大语料 Style RAG 从同一次初稿提示中解耦。`02 风格卡`继续参与初稿提示词组装；`06 风格参考`在初稿（以及可选语言自然化）完成后检索 3～5 个语料片段，并最多调用一次 LLM 完成风格参考重写。移除工作台原“文风校正/严格文风重写”的启用入口和生成链集成。

范围与兼容性：

- `prompt_assembler.py` 仍负责 Style Card 的结构化提示词；正文初稿不再传入 Style Corpus 片段。
- `generation_service.py` 复用现有 `hybrid_search_style`，在已有生成管线内执行 RAG 检索和一次重写，不建立第二套 generation pipeline，也不改变检索排序算法。
- 二次重写必须保留剧情、事实、人物、世界观、已有信息、事件顺序和用户要求，只调整句式、节奏、标点、语言组织、修辞与段落方式；明确禁止复制参考片段中的人物、地点、事件、专有名词和独特长表达。
- 新请求字段为 `style_reference_enabled`。旧 `strict_style_rewrite_enabled` 仅作为向后兼容别名映射到新流程，不再触发 Style Diff 条件判断；现有 `style_rewrite_*` 数据库字段继续承载第二阶段风格参考结果，避免破坏历史记录和新增迁移。
- 提示词预览继续展示初稿提示词，并附加可选的“风格参考二次重写”阶段说明；实际初稿请求严格排除 Style Corpus 片段。
- 工作台节点改为 `02 风格卡` 与 `06 风格参考`；语料库选择移动到 06。05 和 06 同时开启时，执行顺序固定为初稿 → 语言自然化 → 风格参考 → 终稿。

风险与回退：

- 开启 06 必然增加一次模型调用和一次本地检索，带来额外时间与 Token 成本；未选择语料库、没有可用片段或本地检索失败时必须安全跳过并保留上一阶段正文。
- 二次改写可能造成内容漂移，因此重写提示必须使用强约束，测试需验证初稿不含 RAG 片段、RAG 只在初稿后执行且最多一次。
- 旧 API 字段和数据库字段保留，因此可以通过恢复旧生成编排回退，无需迁移或删除任何已有 Style RAG 数据。

验收标准：

- 02 的 Style Card 可独立参与初稿；选中 corpus 但未开启 06 时不检索、不增加 API 调用。
- 开启 06 后，初稿完成才检索语料并执行一次风格参考重写；05 同时开启时以自然化结果作为 06 输入。
- 右侧预览能区分“初稿提示”和“风格参考二次重写”，Token Budget 包含对应独立阶段。
- 工作台节点、连线、状态提示、终稿展示与新语义一致，旧“文风校正/严格文风重写”文案不再出现在工作台。
- 相关 pytest、完整 pytest、Ruff、原生 JavaScript `node --check` 与 `git diff --check` 通过。

运行态复验补充：

- 本机开发端口不得同时运行两份 Flask 服务；重复启动必须立即给出明确错误，避免同一浏览器会话混合读取旧 HTML 与新 JavaScript。
- 04→05→07 与 04→06→07 连线始终可见；未启用时为静止弱化状态，启用且进入对应阶段后才显示流动动画。
- 通过真实 `/api/generation/generate-stream` 请求验证初稿、可选语言自然化、初稿后 RAG 检索、风格参考二次调用及最终成稿的事件顺序，不以源码字符串断言代替运行态验证。
- 通过真实 `/api/generation/preview-prompt` 请求验证预览不触发 Style RAG 检索，避免右栏预览承担重型检索延迟。
- 生成前无法取得依赖初稿的真实 RAG 片段，预览不得再把占位片段伪装成实际 `<style_references>`；应明确标为运行时计划，并在生成完成后反馈真实命中片段数。
- 01/02/03 输入连线分别以“结构化编排已启用 / 智能风格卡已启用 / 上下文非空”为激活依据，和 05/06 一样保持未启用弱化、执行时流动。

完成记录（2026-08-20）：

- 初稿的智能风格链只接收模板级 Style Card/StyleExcerpt，生成与预览路径均不再把 `style_corpus_ids` 交给初稿组装器；
- 新增 06 RAG 风格参考阶段，固定在初稿和可选语言自然化之后检索 4 个片段，并对最新正文最多改写一次；
- 删除工作台旧文风校正开关和 Style Diff 自动改写集成；旧请求字段只作为新开关别名，现有数据库字段继续兼容历史记录；
- 右侧预览增加明确的第二阶段指令区，Token Budget 以独立 `style_reference` 阶段预留 4 个最大 chunk；
- 检索无候选或失败时保留上一阶段正文并向前端提示，不破坏已生成初稿；
- 新增 9 个核心回归测试并更新工作台/SSE 结构测试；完整 pytest `130 passed`，全量 Ruff、全部原生 JavaScript `node --check` 和 `git diff --check` 通过。
- 修复本机 5000 端口可叠加运行新旧服务的问题：重复启动现在会在应用初始化前明确失败，避免旧 HTML 与新 JavaScript 混载；清理旧进程后连续 8 次真实 HTTP 请求均返回新节点文案，且仅剩一个监听进程。
- 后处理连线改为始终呈现：未启用时弱化静止，启用 05/06 且进入对应阶段时才流动；新增 Node 运行态状态测试，不依赖源码字符串断言。
- 新增真实 Flask/SSE 集成测试，验证“初稿 → 语言自然化 → 以自然化稿检索 RAG → 风格参考二次调用 → 最终成稿”的顺序、调用次数和参考片段隔离；真实用户数据下的提示词预览为 5,580 字符、约 126 ms，并确认不执行 RAG。
- 最终完整 pytest `136 passed`，全量 Ruff、16 个原生 JavaScript 文件语法检查与 `git diff --check` 通过。
- 复查真实生成记录 24：语料库 1 实际命中 4 个不同片段并写入 Style Profile 快照，`style_rewrite_applied=true`；确认用户看到的问题来自预览占位文本，而不是 RAG 未执行。
- 预览不再生成伪造的 `<style_references>`，改为独立的“运行时计划”；生成时 SSE 会返回真实命中数量，前端在开始改写和完成时明确提示。
- 01/02/03 连线分别绑定结构化编排开关、智能风格卡模式和上下文非空状态；新增 Node 运行态组合测试。最终完整 pytest `137 passed`，Ruff、16 个 JavaScript 语法检查和 `git diff --check` 通过。
