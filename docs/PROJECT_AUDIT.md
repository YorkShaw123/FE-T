...
> 审计方式：静态只读分析；未运行应用、未调用外部模型、未执行压力测试  
> 说明：本报告中的“问题”和“风险”是基于当前源码的架构判断，不等同于已确认的线上故障。

## 0. 执行摘要

Forestar Editor 是一个以 **Windows 桌面应用为主、Web 自托管为辅** 的本地 AI 文字创作工具。其运行时由 Tauri 桌面壳启动 PyInstaller 打包的 Flask Sidecar，页面使用 Flask 模板和原生 JavaScript，通过本机 HTTP API 与后端交互；业务数据保存在 SQLite，文章生成、风格分析和向量化请求由后端直接调用第三方模型 API。

项目已经形成较清晰的“路由层—服务层—持久化层”结构，并具备模板版本、流式生成、Token 预算、Style Card、Style RAG、局部编辑和历史记录等完整业务能力。当前最值得关注的问题集中在：

1. 桌面端与 Web 端共用同一套后端，但安全边界没有按运行模式分级；Web 模式缺少认证、CSRF 和访问控制。
2. Flask 开发服务器承担桌面和自托管运行，长耗时生成/向量化任务同步占用请求线程。
...
| Web 部署 | Docker | 提供开发调试和高级用户自托管入口 |
...
后端统一封装多家大模型或兼容网关，包括 DeepSeek、OpenAI、硅基流动 Kimi、Gemini 中转和爱化身兼容接口。Style RAG 的 Embedding 默认使用硅基流动的 `BAAI/bge-m3`。
...
| `/api/templates` | 模板 CRUD、示例、版本、导入导出 |
...
- Web 与桌面模式默认共享数据库；旧版项目内 `data/` 首次运行时复制迁移到公共目录。
...
同一个 Flask 应用既服务于仅本机桌面 Sidecar，也允许通过 `FORESTAR_HOST` 对外自托管。API 未看到用户认证、权限模型、CSRF 防护或会话隔离。对 localhost 单用户工具这通常可接受，但一旦绑定 `0.0.0.0`，模板、历史、语料、生成和删除接口都会成为共享能力。

### 7.2 使用 Flask 开发服务器承载实际运行（高）

`app.run()` 同时用于 Web 自托管和打包后的 Sidecar。它不是面向生产部署设计的 WSGI 服务；流式生成、文档解析和批量向量化又是长耗时操作，容易造成请求线程长期占用、并发能力不可控和异常恢复能力不足。
...
| 等级 | 风险 | 触发条件与影响 |
|---|---|---|
| 高 | 未授权访问与破坏性操作 | Web 服务对外监听时，其他访问者可能读取历史/语料，调用付费 API 流程，或执行删除/清空操作。 |
| 高 | 明文 HTTP 传输 API Key | 自托管若跨机器使用且没有 HTTPS 反向代理，API Key 会以请求正文经过明文网络；桌面 localhost 场景风险较低。 |
| 高 | SSRF/网关信任风险 | 若未来开放自定义 `base_url` 或错误传入不可信地址，后端会携带用户凭证请求目标；当前需保持 base URL 白名单边界。 |
| 高 | 上游费用失控 | 生成、去 AI 化、Style Card、片段标注和 Embedding 都可产生费用；缺少用户级额度、并发限制、速率限制和成本日志。 |
| 高 | 长任务失败与结果不一致 | 网络中断或页面取消可能发生上游已经生成、客户端未收到 complete、记录尚未或重复持久化的情况。 |
| 中高 | SQLite 锁竞争或损坏 | 桌面版和 Web 版可同时访问同一数据库；长事务、批量索引、进程异常退出或多实例写入可能产生锁等待和恢复问题。 |
| 中高 | 语料规模导致内存峰值 | 检索时将向量加载为 NumPy 矩阵；语料持续增大后，内存、反序列化和排序成本将线性增长。 |
| 中高 | FTS 索引一致性 | FTS 表需要与 `StyleChunk` 增删保持同步；异常或迁移缺陷可能导致文本召回与主表不一致。 |
...
- 将 desktop/local-only 与 web/self-hosted 设为两套明确安全配置。
- Web 模式增加认证、CSRF、防暴力调用、请求大小限制和反向代理 HTTPS 指南。
- 对删除、清空、向量化和模型调用等高影响接口增加更强的授权与审计。
...
主要架构矛盾不是功能不足，而是**单用户本地工具的实现正在同时承担 Web 自托管、长任务、向量检索和多提供商网关职责**。后续如果仍以桌面单用户为唯一核心场景，可以继续保持轻量，但应强化进程可靠性、数据库保护和外部调用治理；如果准备扩展到局域网、多用户或云端部署，则认证授权、任务队列、生产服务容器、契约化 API 和可扩展存储应先于新功能建设。
...
| 43 | 标记为示例模板 | 保存 payload 的 `is_sample` | 同 #40/#42 | 更新 `PromptTemplate.is_sample`，工作台分组默认排除示例 | 闭环 |
| 44 | 打开示例模板 | `loadSampleTemplates/enterSampleMode` | `GET /api/templates/samples` → `get_sample_templates` | 只读展示内容并生成变量输入框 | 闭环 |
| 45 | 示例另存为模板 | `btn-save-as-template` | `POST /api/templates/from-sample` → `create_template_from_sample` → `fill_variables` | 创建新的普通 `PromptTemplate` | 闭环 |
| 46 | Markdown 编辑/本地预览 | 工具栏、`renderMarkdownPreview` | 无后端调用 | 修改 textarea 并以转义后的简易 Markdown 显示 | 前端本地闭环 |
| 47 | 删除单个普通/示例模板 | 对应 delete button | `DELETE /api/templates/{id}` → `delete_template` | 删除模板；关系配置决定关联 Style 数据级联 | 闭环，UI 有确认框 |
...
- **部署安全边界**：同源写入校验可以阻止普通浏览器跨站攻击，但它不是用户认证。服务一旦显式暴露到局域网/公网，同网访问者仍可能直接调用 API。
